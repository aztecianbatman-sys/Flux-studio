...
QWidget*FluxProductionDock::buildExportTab(){
    auto*w=new QWidget;auto*l=new QVBoxLayout(w);l->addWidget(text("Still, sequence, sprite-sheet and FFmpeg animation export."));auto*g=new QGroupBox("Render settings");auto*f=new QFormLayout(g);m_exportFormat=new QComboBox;m_exportFormat->addItems({"PNG","JPEG","WebP","PNG Sequence","Sprite Sheet","GIF","MP4","WebM"});m_exportWidth=new QSpinBox;m_exportWidth->setRange(1,16384);m_exportWidth->setValue(m_document?m_document->width():1920);m_exportHeight=new QSpinBox;m_exportHeight->setRange(1,16384);m_exportHeight->setValue(m_document?m_document->height():1080);auto*fps=new QSpinBox;fps->setRange(1,240);fps->setValue(24);auto*bitrate=new QSpinBox;bitrate->setRange(100,100000);bitrate->setValue(12000);auto*transparent=new QCheckBox("Preserve transparency");f->addRow("Format",m_exportFormat);f->addRow("Width",m_exportWidth);f->addRow("Height",m_exportHeight);f->addRow("FPS",fps);f->addRow("Bitrate kbps",bitrate);f->addRow(transparent);l->addWidget(g);auto*exportB=action("Export…");l->addWidget(exportB);
    connect(exportB,&QPushButton::clicked,this,[this,transparent,fps,bitrate]{const auto f=QFileDialog::getSaveFileName(this,"Export Render",{},"PNG (*.png);;JPEG (*.jpg);;WebP (*.webp);;GIF (*.gif);;MP4 (*.mp4);;WebM (*.webm)");if(f.isEmpty())return;FluxRenderJob j;j.output=f;j.format=QFileInfo(f).suffix().toLower();j.name=QFileInfo(f).completeBaseName();j.settings.width=m_exportWidth->value();j.settings.height=m_exportHeight->value();j.settings.fps=fps->value();j.settings.startFrame=0;j.settings.endFrame=m_document->frameCount()-1;j.settings.bitrateKbps=bitrate->value();j.settings.transparent=transparent->isChecked();j.settings.codec=j.format=="webm"?"libvpx-vp9":"libx264";QString e;const bool still=j.format=="png"||j.format=="jpg"||j.format=="jpeg"||j.format=="webp";const bool ok=still?FluxExportEngine::exportImage(m_document->composite(),f,j.settings,&e):FluxExportEngine::exportAnimated([this](int frame){m_document->setFrame(frame);return m_document->composite();},j,&e);if(!ok)QMessageBox::critical(this,"Export failed",e);else QMessageBox::information(this,"Flux Studio","Render complete.");});return w;
}
