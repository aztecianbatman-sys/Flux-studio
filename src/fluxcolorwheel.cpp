...
void FluxColorWheel::pick(const QPointF& point){const QPointF c(width()/2.0,height()/2.0);const qreal outer=qMin(width(),height())*0.43;const qreal dx=point.x()-c.x(),dy=point.y()-c.y(),r=qSqrt(dx*dx+dy*dy);if(r>outer*0.72&&r<outer*1.06){qreal h=qAtan2(dy,dx)/(2*M_PI)+0.5;if(h<0)h+=1;int hue=0,sat=0,val=255;m_color.getHsv(&hue,&sat,&val);setColor(hsvColor(h,sat/255.0,val/255.0));return;}const qreal side=outer*1.34;const QRectF area(c.x()-side/2,c.y()-side/2,side,side);if(!area.contains(point))return;const qreal x=qBound<qreal>(0,(point.x()-area.left())/area.width(),1),y=qBound<qreal>(0,(point.y()-area.top())/area.height(),1);int hue=0,sat=0,val=255;m_color.getHsv(&hue,&sat,&val);setColor(hsvColor(hue<0?0:hue/360.0,x,1-y));}
