...
bool writeFile(QDataStream& out,const QString& name,const QByteArray& data,QString* error){
    ...
    if(!data.isEmpty()&&out.writeRawData(data.constData(),data.size())!=data.size()){if(error)*error="Failed to write package payload";return false;}
    return true;
}
...
bool FluxProjectPackage::isPackage(const QString&filePath){
    QFile f(filePath);if(!f.open(QIODevice::ReadOnly))return false;QDataStream in(&f);in.setVersion(QDataStream::Qt_6_5);quint32 magic=0;in>>magic;return in.status()==QDataStream::Ok&&magic==Magic;
}
