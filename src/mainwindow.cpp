...
void FluxMainWindow::buildMenus(){
    ...
    auto* edit=menuBar()->addMenu("Edit"); edit->addAction("Undo",m_canvas,&FluxCanvas::undo,QKeySequence::Undo); edit->addAction("Redo",m_canvas,&FluxCanvas::redo,QKeySequence::Redo);
    edit->addSeparator(); auto* color=edit->addAction("Foreground Color…",this,&FluxMainWindow::chooseColor); Q_UNUSED(color)
    auto* view=menuBar()->addMenu("View"); view->addAction("Fit Canvas",m_canvas,&FluxCanvas::fitCanvas); view->addAction("Fullscreen Canvas",this,[this]{ showFullScreen(); },QKeySequence("Ctrl+Shift+F"));
    view->addAction("Exit Fullscreen",this,[this]{ showNormal(); });
    ...
    auto* animation=menuBar()->addMenu("Animation"); animation->addAction("Play / Pause",this,&FluxMainWindow::togglePlayback,QKeySequence("Space")); animation->addAction("New Frame",this,[this]{m_document->ensureFrame(m_document->frame()); m_document->setFrame(qMin(m_document->frame()+1,m_document->frameCount()-1)); refreshTimeline();m_canvas->update();});
    animation->addAction("Add 12 Frames",this,[this]{m_document->setFrameCount(m_document->frameCount()+12);refreshTimeline();});
    auto* window=menuBar()->addMenu("Window"); window->addAction("Reset Workspace",this,[this]{resize(1540,940);});
    menuBar()->addMenu("Canvas"); menuBar()->addMenu("Select"); menuBar()->addMenu("Filter"); menuBar()->addMenu("Help");
}
...
void FluxMainWindow::buildToolbar(){
    ...
    for(int i=0;i<glyphs.size();++i){auto* b=tool(glyphs[i],tips[i]); b->setProperty("toolName",tips[i]); tb->addWidget(b); connect(b,&QToolButton::clicked,this,&FluxMainWindow::setToolFromAction);}
    auto* spacer=new QWidget; spacer->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Preferred); tb->addWidget(spacer);
    auto* colorBtn=new QToolButton; colorBtn->setText("●"); colorBtn->setToolTip("Foreground color"); colorBtn->setObjectName("colorButton"); tb->addWidget(colorBtn); connect(colorBtn,&QToolButton::clicked,this,&FluxMainWindow::chooseColor);
    tb->addSeparator(); tb->addWidget(new QLabel(" Size "));
    m_brushSlider=new QSlider(Qt::Horizontal); m_brushSlider->setRange(1,500); m_brushSlider->setValue(24); m_brushSlider->setFixedWidth(150); tb->addWidget(m_brushSlider); connect(m_brushSlider,&QSlider::valueChanged,this,&FluxMainWindow::updateBrushSize);
    tb->addWidget(new QLabel(" px"));
}
...
void FluxMainWindow::buildDocks(){
    ...
    auto* timelineDock=new QDockWidget("Timeline",this); timelineDock->setObjectName("TimelineDock"); timelineDock->setWidget(makeTimelinePanel()); addDockWidget(Qt::BottomDockWidgetArea,timelineDock); timelineDock->setMinimumHeight(250);
}

QWidget* FluxMainWindow::makeLayersPanel(){
    auto* root=new QWidget; auto* lay=new QVBoxLayout(root); lay->setContentsMargins(10,10,10,10); lay->addWidget(title("LAYERS"));
    m_layers=new QTreeWidget; m_layers->setHeaderLabels({"Layer","Opacity"}); m_layers->header()->setStretchLastSection(false); m_layers->header()->setSectionResizeMode(0,QHeaderView::Stretch); m_layers->header()->setSectionResizeMode(1,QHeaderView::ResizeToContents); lay->addWidget(m_layers,1);
    connect(m_layers,&QTreeWidget::currentItemChanged,this,[this](QTreeWidgetItem* current,QTreeWidgetItem*){ if(!current)return; bool ok=false; int idx=current->data(0,Qt::UserRole).toInt(&ok);if(ok){m_document->setActiveLayer(idx);m_canvas->update();} });
    auto* row=new QHBoxLayout; auto* add=new QPushButton("＋"); auto* dup=new QPushButton("⧉"); auto* rem=new QPushButton("−"); row->addWidget(add);row->addWidget(dup);row->addWidget(rem);row->addStretch(); lay->addLayout(row);
    connect(add,&QPushButton::clicked,this,&FluxMainWindow::addLayer); connect(dup,&QPushButton::clicked,this,&FluxMainWindow::duplicateLayer); connect(rem,&QPushButton::clicked,this,&FluxMainWindow::removeLayer); return root;
}

QWidget* FluxMainWindow::makeInspectorPanel(){
    auto* root=new QWidget; auto* lay=new QVBoxLayout(root); lay->setContentsMargins(12,12,12,12); lay->addWidget(title("INSPECTOR"));
    auto* form=new QFormLayout; auto* size=new QSpinBox; size->setRange(1,500); size->setValue(24); form->addRow("Brush size",size); connect(size,qOverload<int>(&QSpinBox::valueChanged),this,&FluxMainWindow::updateBrushSize);
    auto* opacity=new QSlider(Qt::Horizontal); opacity->setRange(0,100);opacity->setValue(100);form->addRow("Opacity",opacity);
    auto* spacing=new QSlider(Qt::Horizontal);spacing->setRange(1,100);spacing->setValue(12);form->addRow("Spacing",spacing);
    auto* stabilizer=new QSlider(Qt::Horizontal);stabilizer->setRange(0,100);stabilizer->setValue(20);form->addRow("Stabilizer",stabilizer);lay->addLayout(form);
    lay->addSpacing(12);lay->addWidget(new QLabel("Brush presets"));auto* presets=new QListWidget; for(const auto& s:QStringList{"Flux Basic","Soft Ink","Pencil HB","Opaque Ink","Airbrush","Marker","Chalk","Pixel"}) presets->addItem(s);lay->addWidget(presets,1); return root;
}

QWidget* FluxMainWindow::makeTimelinePanel(){
    auto* root=new QWidget;auto* lay=new QVBoxLayout(root);lay->setContentsMargins(10,8,10,8);auto* head=new QHBoxLayout;head->addWidget(title("TIMELINE"));head->addSpacing(18);head->addWidget(new QLabel("FPS"));m_fps=new QSpinBox;m_fps->setRange(1,240);m_fps->setValue(24);head->addWidget(m_fps);head->addWidget(new QLabel("   Frame"));head->addStretch();auto* play=new QPushButton("▶  Play");auto* stop=new QPushButton("■");head->addWidget(play);head->addWidget(stop);lay->addLayout(head);
    m_frames=new QListWidget;m_frames->setFlow(QListView::LeftToRight);m_frames->setWrapping(false);m_frames->setSpacing(2);connect(m_frames,&QListWidget::currentRowChanged,this,&FluxMainWindow::setFrameFromTimeline);lay->addWidget(m_frames,1);
    connect(play,&QPushButton::clicked,this,&FluxMainWindow::togglePlayback);connect(stop,&QPushButton::clicked,this,[this]{m_playing=false;m_playTimer->stop();setStatus("Playback stopped");});return root;
}

void FluxMainWindow::refreshLayers(){ if(!m_layers)return;m_layers->clear();for(int i=m_document->layers().size()-1;i>=0;--i){const auto& l=m_document->layers()[i];auto* item=new QTreeWidgetItem({l.name,QString::number(int(l.opacity*100))+"%"});item->setData(0,Qt::UserRole,i);item->setCheckState(0,l.visible?Qt::Checked:Qt::Unchecked);m_layers->addTopLevelItem(item);}if(m_document->activeLayerIndex()<m_layers->topLevelItemCount())m_layers->setCurrentItem(m_layers->topLevelItem(m_layers->topLevelItemCount()-1-m_document->activeLayerIndex()));}
...
void FluxMainWindow::newProject(){int w=1920,h=1080;bool ok=false;const QString size=QInputDialog::getText(this,"New Flux Project","Canvas size (width x height)",QLineEdit::Normal,"1920 x 1080",&ok);if(ok){const auto parts=size.split('x');if(parts.size()==2){w=parts[0].trimmed().toInt(&ok);h=parts[1].trimmed().toInt(&ok);}}if(ok&&w>0&&h>0){m_document->create("Untitled",w,h);m_filePath.clear();m_canvas->setDocument(m_document);refreshLayers();refreshTimeline();setStatus("● Unsaved");}}
void FluxMainWindow::openProject(){const QString path=QFileDialog::getOpenFileName(this,"Open Flux Project",QString(),"Flux Project (*.flux);;JSON Project (*.json)");if(path.isEmpty())return;QString err;if(!m_document->load(path,&err)){QMessageBox::critical(this,"Flux Studio",err);return;}m_filePath=path;m_canvas->setDocument(m_document);refreshLayers();refreshTimeline();setStatus("✓ Saved");QSettings().setValue("lastProject",path);}
...
void FluxMainWindow::autosave(){if(!m_filePath.isEmpty())m_autosavePath=m_filePath+QStringLiteral(".autosave.flux");else m_autosavePath=QDir::temp().filePath("flux-studio-autosave.flux");QString err;m_document->save(m_autosavePath,&err);}
...
void FluxMainWindow::restoreLastSession(){const QString path=QSettings().value("lastProject").toString();if(path.isEmpty()||!QFileInfo::exists(path))return;}
