...
namespace { QLabel* title(const QString& text){auto*l=new QLabel(text);l->setObjectName("panelTitle");return l;} QToolButton* tool(const QString& glyph,const QString& tip){auto*b=new QToolButton;b->setText(glyph);b->setToolTip(tip);b->setCheckable(true);b->setAutoExclusive(true);b->setObjectName("toolButton");return b;} }

FluxMainWindow::FluxMainWindow(QWidget* parent):QMainWindow(parent){
    setWindowTitle("Flux Studio");resize(1540,940);setDockNestingEnabled(true);m_document=new FluxDocument();m_canvas=new FluxCanvas(this);m_canvas->setDocument(m_document);setCentralWidget(m_canvas);buildMenus();buildToolbar();buildDocks();
    auto*wheel=new FluxWheel(m_canvas);wheel->raise();connect(m_canvas,&FluxCanvas::wheelRequested,wheel,&FluxWheel::openAt);connect(m_canvas,&FluxCanvas::documentChanged,this,&FluxMainWindow::markModified);connect(m_canvas,&FluxCanvas::cursorInfoChanged,this,[this](const QString&s){if(m_cursorLabel)m_cursorLabel->setText(s);});
    ...
    m_statusLabel=new QLabel("✓ Saved");m_cursorLabel=new QLabel("X 0  Y 0");statusBar()->addPermanentWidget(m_statusLabel);statusBar()->addPermanentWidget(m_cursorLabel);polish();refreshLayers();refreshTimeline();restoreLastSession();
...
void FluxMainWindow::buildMenus(){
    ...
    auto*animation=menuBar()->addMenu("Animation");animation->addAction("Play / Pause",this,&FluxMainWindow::togglePlayback,QKeySequence(Qt::Key_Space));animation->addAction("Duplicate Frame",this,[this]{const int current=m_document->frame();const int target=qMin(current+1,m_document->frameCount()-1);for(auto&layer:m_document->layers()){if(layer.frames.size()<m_document->frameCount())layer.frames.resize(m_document->frameCount());if(target!=current)layer.frames[target]=(layer.frames.size()>current&&!layer.frames[current].isNull())?layer.frames[current].copy():layer.image.copy();}m_document->setFrame(target);refreshTimeline();m_canvas->update();markModified();});animation->addAction("Next Frame",this,[this]{m_document->setFrame(qMin(m_document->frame()+1,m_document->frameCount()-1));refreshTimeline();m_canvas->update();});animation->addAction("Add 12 Frames",this,[this]{m_document->setFrameCount(m_document->frameCount()+12);refreshTimeline();});
    menuBar()->addMenu("Canvas");menuBar()->addMenu("Select");menuBar()->addMenu("Filter");menuBar()->addMenu("Window");menuBar()->addMenu("Help");
}

void FluxMainWindow::buildToolbar(){auto*tb=addToolBar("Tools");tb->setMovable(false);const QStringList glyphs={"✦","✎","▣","⌫","▤","↗","○","△","⌁","◉","□","⌕","✋"};const QStringList tips={"Brush","Pencil","Ink","Eraser","Fill","Transform","Ellipse","Polygon","Bezier","Color Picker","Select","Zoom","Pan"};for(int i=0;i<glyphs.size();++i){auto*b=tool(glyphs[i],tips[i]);b->setProperty("toolName",tips[i]);tb->addWidget(b);connect(b,&QToolButton::clicked,this,&FluxMainWindow::setToolFromAction);}auto*spacer=new QWidget;spacer->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Preferred);tb->addWidget(spacer);auto*colorBtn=new QToolButton;colorBtn->setText("●");colorBtn->setToolTip("Foreground color");colorBtn->setObjectName("colorButton");tb->addWidget(colorBtn);connect(colorBtn,&QToolButton::clicked,this,&FluxMainWindow::chooseColor);tb->addSeparator();tb->addWidget(new QLabel(" Size "));m_brushSlider=new QSlider(Qt::Horizontal);m_brushSlider->setRange(1,500);m_brushSlider->setValue(24);m_brushSlider->setFixedWidth(150);tb->addWidget(m_brushSlider);connect(m_brushSlider,&QSlider::valueChanged,this,&FluxMainWindow::updateBrushSize);tb->addWidget(new QLabel(" px"));}
...
QWidget* FluxMainWindow::makeLayersPanel(){auto*root=new QWidget;auto*lay=new QVBoxLayout(root);lay->setContentsMargins(10,10,10,10);lay->addWidget(title("LAYERS"));m_layers=new QTreeWidget;m_layers->setHeaderLabels({"Layer","Opacity"});m_layers->header()->setStretchLastSection(false);m_layers->header()->setSectionResizeMode(0,QHeaderView::Stretch);m_layers->header()->setSectionResizeMode(1,QHeaderView::ResizeToContents);lay->addWidget(m_layers,1);connect(m_layers,&QTreeWidget::itemChanged,this,[this](QTreeWidgetItem*item,int column){if(column!=0||!item)return;bool ok=false;int idx=item->data(0,Qt::UserRole).toInt(&ok);if(ok&&idx>=0&&idx<m_document->layers().size()){m_document->layers()[idx].visible=item->checkState(0)==Qt::Checked;m_canvas->update();markModified();}});connect(m_layers,&QTreeWidget::currentItemChanged,this,[this](QTreeWidgetItem*current,QTreeWidgetItem*){if(!current)return;bool ok=false;int idx=current->data(0,Qt::UserRole).toInt(&ok);if(ok){m_document->setActiveLayer(idx);m_canvas->update();}});auto*row=new QHBoxLayout;auto*add=new QPushButton("＋");auto*dup=new QPushButton("⧉");auto*rem=new QPushButton("−");row->addWidget(add);row->addWidget(dup);row->addWidget(rem);row->addStretch();lay->addLayout(row);connect(add,&QPushButton::clicked,this,&FluxMainWindow::addLayer);connect(dup,&QPushButton::clicked,this,&FluxMainWindow::duplicateLayer);connect(rem,&QPushButton::clicked,this,&FluxMainWindow::removeLayer);return root;}

QWidget* FluxMainWindow::makeInspectorPanel(){auto*root=new QWidget;auto*lay=new QVBoxLayout(root);lay->setContentsMargins(12,12,12,12);lay->addWidget(title("INSPECTOR"));auto*form=new QFormLayout;auto*size=new QSpinBox;size->setRange(1,500);size->setValue(24);form->addRow("Brush size",size);connect(size,qOverload<int>(&QSpinBox::valueChanged),this,&FluxMainWindow::updateBrushSize);auto*opacity=new QSlider(Qt::Horizontal);opacity->setRange(0,100);opacity->setValue(100);form->addRow("Opacity",opacity);auto*spacing=new QSlider(Qt::Horizontal);spacing->setRange(1,100);spacing->setValue(12);form->addRow("Spacing",spacing);auto*stabilizer=new QSlider(Qt::Horizontal);stabilizer->setRange(0,100);stabilizer->setValue(20);form->addRow("Stabilizer",stabilizer);auto*onion=new QCheckBox("Onion skin");onion->setChecked(true);form->addRow("Animation",onion);connect(onion,&QCheckBox::toggled,m_canvas,&FluxCanvas::toggleOnionSkin);lay->addLayout(form);lay->addSpacing(12);lay->addWidget(new QLabel("Brush presets"));auto*presets=new QListWidget;for(const auto&s:QStringList{"Flux Basic","Soft Ink","Pencil HB","Opaque Ink","Airbrush","Marker","Chalk","Pixel"})presets->addItem(s);lay->addWidget(presets,1);return root;}
...
void FluxMainWindow::refreshLayers(){if(!m_layers)return;QSignalBlocker b(m_layers);m_layers->clear();for(int i=m_document->layers().size()-1;i>=0;--i){const auto&l=m_document->layers()[i];auto*item=new QTreeWidgetItem({l.name,QString::number(int(l.opacity*100))+"%"});item->setData(0,Qt::UserRole,i);item->setCheckState(0,l.visible?Qt::Checked:Qt::Unchecked);m_layers->addTopLevelItem(item);}if(m_layers->topLevelItemCount()>0)m_layers->setCurrentItem(m_layers->topLevelItem(qMax(0,m_layers->topLevelItemCount()-1-m_document->activeLayerIndex())));}
void FluxMainWindow::refreshTimeline(){if(!m_frames)return;QSignalBlocker b(m_frames);m_frames->clear();for(int i=0;i<m_document->frameCount();++i){auto*item=new QListWidgetItem(QString::number(i+1));item->setSizeHint(QSize(52,42));item->setTextAlignment(Qt::AlignCenter);m_frames->addItem(item);}m_frames->setCurrentRow(m_document->frame());}
...
void FluxMainWindow::togglePlayback(){m_playing=!m_playing;if(m_playing){m_playTimer->setInterval(qMax(1,1000/m_fps->value()));m_playTimer->start();setStatus("▶ Playing");}else{m_playTimer->stop();setStatus("❚❚ Paused");}}
...
void FluxMainWindow::restoreLastSession(){const QString path=QSettings().value("lastProject").toString();if(!path.isEmpty()&&QFileInfo::exists(path))statusBar()->showMessage(QString("Last project available: %1").arg(QFileInfo(path).fileName()),4000);const QByteArray state=QSettings().value("workspace").toByteArray();if(!state.isEmpty())restoreState(state);}
