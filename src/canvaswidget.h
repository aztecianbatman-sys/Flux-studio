...
signals:
    void wheelRequested(const QPoint& globalPos); void brushSizeChanged(int px); void documentChanged(); void cursorInfoChanged(const QString& text); void strokeStarted(); void selectionChanged(); void zoomChanged(double zoom); void referenceChanged();
protected:
...
private:
    ...
    QPointF m_lastPoint,m_cursor,m_selectionStart,m_transformStart; QVector<QPointF> m_lasso; bool m_drawing=false,m_selecting=false,m_onionSkin=true,m_grid=true,m_rulers=true,m_perspective=false,m_transforming=false,m_symmetryH=false,m_symmetryV=false;
    int m_brushSize=24; qreal m_stabilization=.12; QColor m_brushColor=Qt::black; QString m_tool=QStringLiteral("Brush"); qreal m_pressure=1.0,m_tiltX=0,m_tiltY=0,m_rotationInput=0; QImage m_referenceImage; QPointF m_referencePosition; QVector<QImage> m_undo,m_redo;
};
