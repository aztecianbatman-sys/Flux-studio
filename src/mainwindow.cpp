...
FluxMainWindow::FluxMainWindow(QWidget* parent):QMainWindow(parent){
    ...
    m_document=new FluxDocument();m_canvas=new FluxCanvas(this);m_canvas->setDocument(m_document);setCentralWidget(m_canvas);buildMenus();buildToolbar();buildDocks();
...
void FluxMainWindow::buildMenus(){
    ...
    auto* animation=menuBar()->addMenu("Animation");animation->addAction("Play / Pause",this,&FluxMainWindow::togglePlayback,QKeySequence(Qt::Key_Space));animation->addAction("Duplicate Frame",this,[this]{const int current=m_document->frame();for(auto& layer:m_document->layers()){m_document->setFrame(current);if(layer.frames.size()<m_document->frameCount())layer.frames.resize(m_document->frameCount());if(current+1<m_document->frameCount())layer.frames[current+1]=m_document->activeImage().copy();}m_document->setFrame(qMin(current+1,m_document->frameCount()-1));refreshTimeline();m_canvas->update();markModified();});animation->addAction("Next Frame",this,[this]{m_document->setFrame(qMin(m_document->frame()+1,m_document->frameCount()-1));refreshTimeline();m_canvas->update();});animation->addAction("Add 12 Frames",this,[this]{m_document->setFrameCount(m_document->frameCount()+12);refreshTimeline();});
